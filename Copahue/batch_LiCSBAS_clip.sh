...
p05_clip_range_geo=-71.4082/-70.9578/-38.0812/-37.6308 #""	# e.g. 130.11/131.12/34.34/34.6 (in deg)
...
p01_frame="083D_12832_131313"	# e.g. 021D_04972_131213 
...
if [ $start_step -le 15 -a $end_step -ge 15 ];then
  ...
  if [ "$check_only" == "y" ];then
    ...
    python /gws/ssde/j25a/nceo_geohazards/vol1/projects/COMET/DEEPVolc_Pedro/SCRIPTS/LiCSBAS15_mask_ts.py $p15_op 2>&1 | tee -a $log
    if [ ${PIPESTATUS[0]} -ne 0 ];then exit 1; fi
  fi
fi
